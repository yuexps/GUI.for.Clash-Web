...
const taskName = ref(scheduledTasksStore.getScheduledTaskById(props.id)?.name)
const keywords = ref('')
...
const filteredLogs = computed(() => {
  return logsStore.scheduledtasksLogs.filter((v) => {
    const p = taskName.value ? v.name === taskName.value : true
...
<template>
  <div class="h-full flex flex-col">
    <div class="flex items-center">
      ...
      <Select v-model="taskName" :options="tasksOptions" size="small" />
      <Input
        v-model="keywords"
        clearable
        size="small"
