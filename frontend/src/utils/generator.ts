...
export const generateConfig = async (originalProfile: IProfile) => {
  ...
  // 智能 DNS 优化：防止 TUN 模式下的域名解析循环依赖死锁
  if (result.dns && result.dns.enable) {
    // 1. 优化 nameserver
    const ns = result.dns.nameserver || []
    if (Array.isArray(ns)) {
      const hasDirectDNS = ns.some((dns: string) => dns && !dns.includes('#'))
      if (!hasDirectDNS) {
        ns.unshift('223.5.5.5', '119.29.29.29')
        result.dns.nameserver = ns
      }
    } else if (typeof ns === 'string' && ns.includes('#')) {
      result.dns.nameserver = ['223.5.5.5', '119.29.29.29', ns]
    }

    // 2. 优化 default-nameserver (用于解析 DoT/DoH 域名的基础 DNS)
    const defaultNs = result.dns['default-nameserver'] || []
    if (Array.isArray(defaultNs)) {
      const hasDirectDefault = defaultNs.some((dns: string) => dns && !dns.includes('#'))
      if (!hasDirectDefault) {
        defaultNs.unshift('223.5.5.5', '119.29.29.29')
        result.dns['default-nameserver'] = defaultNs
      }
    } else if (typeof defaultNs === 'string' && defaultNs.includes('#')) {
      result.dns['default-nameserver'] = ['223.5.5.5', '119.29.29.29', defaultNs]
    }
  }
