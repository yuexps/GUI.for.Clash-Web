import { apiRequest, EventsOn, EventsOff, EventsEmit } from './request'
...
const requestWithProgress = (fnName: 'Download' | 'Upload') => {
  return async (
    ...
    const {
      ...
      headers: respHeaders,
      body: respBody,
    } = await apiRequest(`net/${fnName.toLowerCase()}`, {
      method,
      url: transformRequestUrl(url),
      path,
      headers: _headers,
      event: progressEvent,
      options: _options,
...
export const TcpPing = async (address: string, options: NetOptions = {}) => {
  const { flag, data } = await apiRequest('net/ping', { address, options: mergeNetOptions(options) })
  if (!flag) throw data
  return Number(data)
}

export const TcpRequest = async (address: string, payload: string, options: NetOptions = {}) => {
  const { flag, data } = await apiRequest('net/tcprequest', { address, payload, options: mergeNetOptions(options) })
  if (!flag) throw data
  return data
}

export const UdpRequest = async (address: string, payload: string, options: NetOptions = {}) => {
  const { flag, data } = await apiRequest('net/udprequest', { address, payload, options: mergeNetOptions(options) })
  if (!flag) throw data
  return data
}
